...
public class MinetunesGui extends GuiScreen {
		...
		if (outdated) {
			autoUpdateButton.displayString = "§aUpdate: "
					+ Minetunes.autoUpdater
							.getLatestVersion(TDConfig.MC_CURRENT_VERSION);
		}
	...
	protected void actionPerformed(GuiButton guibutton) {
		if (guibutton.id == 100) {
			...
			// Toggle MineTunes on
			TDConfig.incrementMCDittyOffState();
			turnedOffButton.displayString = TDConfig
	...
	private void openMidiFolder() {
		String fileLocation = (TDConfig.getMidiDir()).getAbsolutePath();
		TDConfig.getMidiDir().mkdirs();
	...
	public void initGui() {
		...
		final TuneTileGui noteblockTile = new TuneTileGui(width / 4 - 32,
				...
				!TDConfig.getBoolean("noteblock.signsDisabled"), true);
		...
		final TuneTileGui blockTunesTile = new TuneTileGui(width / 2 - 32,
				...
				!TDConfig.getBoolean("blockTunes.disabled"), true);
		...
		final TuneTileGui signTunesTile = new TuneTileGui(width / 4 * 3 - 32,
				...
				!TDConfig.getBoolean("signs.disabled"));
		...
		Thread t = new Thread(new Runnable() {
			...
			public void run() {
				...
				MinetunesGui.setTutorialUpdated(Minetunes.tutorialUpdater
						.checkForUpdates(TDConfig.MC_CURRENT_VERSION));
			}
