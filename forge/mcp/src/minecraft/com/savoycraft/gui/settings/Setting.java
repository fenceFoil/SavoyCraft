...
public class Setting extends Gui {
	...
	public void draw(int mx, int my) {
		...
		if (button != null) {
			if (button instanceof GuiSlider) {
				...
				s.sliderValue = TDConfig.getFloat(settingKey
						+ ".sliderPos");
	...
	private Object getValueObj() {
		...
		case BOOLEAN_ON_OFF:
		case BOOLEAN_YES_NO:
			return TDConfig.getBoolean(settingKey);
		case COLOR:
			return TDConfig.getFloat(settingKey + ".sliderPos");
		case INTEGER_SHORT_TIME:
			return TDConfig.getInt(settingKey);
		case MIDI_VOLUME:
			return TDConfig.getVolumeMode();
		case NO_PLAY_TOKENS:
			return null;
		default:
			break;
	...
	public void onMousePressed(int mx, int my, int mb) {
		if (button != null) {
			...
			if (clicked) {
				...
				case BOOLEAN_ON_OFF:
				case BOOLEAN_YES_NO:
					TDConfig.toggleBoolean(settingKey);
					break;
				case COLOR:
					sliderClicked = true;
					break;
				case INTEGER_SHORT_TIME:
					TDConfig.setInt(settingKey, SettingType
							.nextShortTimeIntValue(TDConfig
									.getInt(settingKey)));
					break;
				case MIDI_VOLUME:
					TDConfig.incrementVolumeMode();
					break;
				case NO_PLAY_TOKENS:
				...
				TDConfig
						.setString(
								...
								new Color4f(
										...
										.toString());
				TDConfig.setFloat(settingKey + ".sliderPos",
		...
		case BOOLEAN_YES_NO:
			button = new GuiButton(settingKey.hashCode(), 0, -50, 100, 20,
					type.getButtonLabel(type,
							TDConfig.getBoolean(settingKey),
							valueInverted));
			break;
		case COLOR:
			button = new GuiSlider(settingKey.hashCode(), 0, -50, "",
					TDConfig.getFloat(settingKey + ".sliderPos"));
			break;
		case INTEGER_SHORT_TIME:
			button = new GuiButton(settingKey.hashCode(), 0, -50, 100, 20,
					type.getButtonLabel(type,
							TDConfig.getInt(settingKey), valueInverted));
			break;
		case MIDI_VOLUME:
			button = new GuiButton(settingKey.hashCode(), 0, -50, 100, 20,
					type.getButtonLabel(
