...
public class UpdateResourcesThread extends Thread {
	...
	public synchronized void run() {
		...
		if (TDConfig.getInt("resources.lastZipDownloaded") == resourcesVersion) {
		...
		File resourcesDir = TDConfig.getResourcesDir();
		...
		ResourceManager.registerSoundResources();

		// Note success
		TDConfig.setBoolean("resources.missing", false);
		TDConfig.setInt("resources.lastZipDownloaded", resourcesVersion);
		try {
			TDConfig.flush();
		} catch (IOException e) {
			e.printStackTrace();
	...
	private void fail() {
		...
		// Note the failure
		TDConfig.setBoolean("resources.missing", true);
		try {
			TDConfig.flush();
		} catch (IOException e) {
			e.printStackTrace();
